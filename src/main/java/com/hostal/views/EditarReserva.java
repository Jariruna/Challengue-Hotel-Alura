...
import com.hostal.modelo.PrecioPorDia;
...
public class EditarReserva extends JFrame {
	...
	public EditarReserva(Reserva reserva) {
		...
		JLabel lblTitulo = new JLabel("EDICION DE RESERVAS");
		...
		txtFechaEntrada.getCalendarButton().setIcon(new ImageIcon(EditarReserva.class.getResource("/imagenes/icon-reservas.png")));
		...
		txtFechaSalida.getCalendarButton().setIcon(new ImageIcon(EditarReserva.class.getResource("/imagenes/icon-reservas.png")));
	...
	protected void actualizarReserva(Reserva reserva) {
		...
		if( fechaIngreso.isBlank() ||  fechaSalida.isBlank() || valor.isBlank()) {
		...
		if(dateIngreso.compareTo(dateSalida)>=0) {
		...
		JOptionPane.showMessageDialog(contentPane,"Reserva actualizada con éxito id:"+reserva.getId());
	...
	private void llenarForma(Reserva reserva) {
	...
	protected void calcularPrecio() {
		...
		if( fechaIngreso.isBlank() ||  fechaSalida.isBlank() ) {
			txtValor.setText("");
			return;
		}
		...
		long diff=dateSalida.getTime()-dateIngreso.getTime();
		...
		Double precio=Double.valueOf(dias)*PrecioPorDia.getPrecioPorDia();
